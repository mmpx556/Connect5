...
function checkWin(r, c, highlight = false){
  ...
  for(const [dr, dc] of dirs){
    ...
    if(count >= 5){
      if(highlight) winCells.forEach(([rr, cc]) => cells[rr][cc].classList.add("highlight"));
      return true;
    }
  }
  return false;
...
function computerMove(){
  ...
  if(turnNumber === 1 && currentPlayer === 1){
    ...
    if(rep && !board[rep[0]][rep[1]]){ makeMove(rep[0], rep[1], 1); return; }
  }
  // 1. tss
...
function tssDFS(maxDepth, timeLimit = 500){
